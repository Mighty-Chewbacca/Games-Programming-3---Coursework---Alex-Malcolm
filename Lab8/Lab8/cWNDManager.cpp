...
LRESULT CALLBACK cWNDManager::WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) //this method meddles with the windows procedures
{
	...
	case WM_KEYDOWN:
	{
		//if (wParam == VK_ESCAPE) //If the escape key was pressed //DISABLING THIS TO ALLOW FOR MY PAUSE MENU
		//{
		//	exit(1); //this stoppped my display drivers from crashing when exiting
	...
	case WM_MOUSEMOVE:
	{
			 ...
			 //std::cout << "the mouse moved: " << "X:" << LOWORD(lParam) << " Y:" << HIWORD(lParam) <<  std::endl;
			 return 0;
	}
		break;
